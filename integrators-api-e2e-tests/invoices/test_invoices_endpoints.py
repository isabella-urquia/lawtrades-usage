...
def test_get_all_invoices_endpoint():
    """
    Test the GET /invoices endpoint for retrieving paginated customer data.
    ...
    """
    endpoint = "/invoices"  # Endpoint for retrieving customers
...
@pytest.mark.integration
def test_get_invoices_by_search_criteria():
    """
    Test the GET /invoices endpoint with search criteria.
    ...
    url = f"{TestBase.BASE_URL}{endpoint}"
    params = {
        "filter": "customerId:eq:2dd88c8f-adf6-4b4e-a141-b6fe6f220732"
    }
    response = requests.get(url, headers=TestBase.get_headers(), params=params)
    assert response.status_code == 200, f"Expected 200 OK, got {response.status_code}"
    json_response = response.json()
