...
def customer_id(vid):
    cursor.execute("SELECT c.customer_id FROM contracts c join customers cu on c.customer_id = cu.id join customer_external_ids cei on cei.customer_id = cu.id where cu.manufacturer_id = '76310fa7-758a-4062-307e-a9e75497b770' and cei.source_type = 'QUICKBOOKS' and cei.customer_external_type='VENDOR' and c.deleted_at is NULL and cei.external_id = '{}' limit 1".format(vid))
    row = cursor.fetchone()
    # Transform the result into a dictionary
    return row[0] if row else None
...
if __name__ == "__main__":
    dicts = csv_to_list_of_dicts("/Users/chiragdas/Downloads/Findigs_Jan_original.csv")
    vendor_dict = {}
    ...
    for multiple_lines in vendor_dict:
        ...
        for line in vendor_dict[multiple_lines]:
            ...
            if contract == None:
                ...
                continue
            customer =customer_id(multiple_lines)
            if customer == None:
                print("no customer for " + multiple_lines)
                continue
            ...
            csv_dict = {
                ...
                "manuafcturer_id": '76310fa7-758a-4062-307e-a9e75497b770'
            }
            dict_csvs.append(csv_dict)
    
    filename = 'Jan_Findigs_Remittances_2.csv'
