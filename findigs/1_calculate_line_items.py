...
def contract_id(vid):
    cursor.execute("SELECT c.id FROM contracts c join customers cu on c.customer_id = cu.id join customer_external_ids cei on cei.customer_id = cu.id where cu.manufacturer_id = '76310fa7-758a-4062-307e-a9e75497b770' and cei.source_type = 'QUICKBOOKS' and cei.customer_external_type='VENDOR' and cei.external_id = '{}' limit 1".format(vid))
    row = cursor.fetchone()
    # Transform the result into a dictionary
    return row[0] if row else None
