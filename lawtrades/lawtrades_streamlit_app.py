...
def upload_attachment(customer_id, invoice_id, filepath, talent_name=None):
    ...
    try:
        # Get API configuration from session state or environment
        api_key = st.session_state.get('api_key', '') or os.getenv('TABS_API_KEY', '')
        environment = st.session_state.get('environment', 'Sandbox')
        
        if not api_key:
            st.error("API key not configured. Please provide API key in session or set TABS_API_KEY environment variable.")
            return False
